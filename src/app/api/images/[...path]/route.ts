...
export async function GET(
  ...
  try {
    ...
    // Support either query param ?url=https://... or path parsing
    let imageUrl = searchParams.get('url');
    if (!imageUrl && pathParams && pathParams.length > 0) {
      // Reconstruct the path. If it contains "http" or "https" as the first part, reconstruct url
      const pathStr = pathParams.join('/');
      if (pathStr.startsWith('http:/') || pathStr.startsWith('https:/')) {
        imageUrl = pathStr.replace('http:/', 'http://').replace('https:/', 'https://');
      } else if (pathStr.startsWith('http') && !pathStr.includes(':/')) {
        // Fallback for some URL encoding anomalies
        imageUrl = pathStr;
      } else {
    ...
    if (!imageUrl) {
      return new NextResponse('Missing image URL or path', { status: 400 });
    }
